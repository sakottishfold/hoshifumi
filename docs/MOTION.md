...
- **Soft glow(極小 box-shadow / opacity halo の遷移)** ─ 星の光のゆらぎ。amber accent との相性が良い
- **Bloom(opacity + scale 0→1 の一回限りの appear)** ─ 月がそっと開く、星が初めて灯る瞬間。位置の移動はなく、その場で「現れる」だけ。one-time、infinite loop しない。fade の延長として許容(2026-05-18 追加、Star Bloom 機能)
- **Glow-pulse(box-shadow blur radius の breath loop)** ─ bloom 完了後の月相を「灯」として持続させる infinite。breath が形の呼吸なのに対し、glow-pulse は光のゆらぎの呼吸(2026-05-18 追加)
- **Fade-in-soft(opacity + 小 scale 0.6→1 の pop-in)** ─ 小さな光が次々に灯る milestone burst で使う。stagger(100ms 間隔)で複数発生させ、星座が編まれていく印象。位置移動なし(2026-05-18 追加)
...
| **Bloom**(BloomMoon ヒーロー、milestone 中心月の「開く」appear) | `opacity 0 → 1` + `transform: scale(0 → 1)` | 1500ms | `cubic-bezier(0.16, 1, 0.3, 1)` forwards | one-time。位置移動なし、その場で開く。infinite loop しない。`.bloom-animate` utility(`app/globals.css`) |
| **Glow-pulse**(bloom 完了後の月相の持続的な「灯」感) | `box-shadow` blur radius の breath | 3000ms full cycle | ease-in-out infinite | bloom 終了後に切り替えて適用。色は amber `#f5d49a` halo。複数同時起動は避ける(主役 1要素のみ) |
| **Fade-in-soft**(milestone burst の small light 個別発生) | `opacity 0 → 1` + `transform: scale(0.6 → 1)` | 800ms | ease-out forwards | stagger 100ms 間隔で複数個。位置移動なし、その場で pop-in。`.fade-in-soft-animate` utility |
...
### Bloom / glow-pulse / fade-in-soft(Star Bloom 機能、2026-05-18 追加)

`/today/done` の BloomMoon ヒーローと milestone burst で使う。`app/globals.css` に keyframes + utility class が実装済み(`.bloom-animate` / `.glow-animate` / `.fade-in-soft-animate`)。`@media (prefers-reduced-motion: reduce)` で全 animation 無効化も同所で吸収。
...
@keyframes glow-pulse {
  0%, 100% { box-shadow: 0 0 20px 4px rgba(245, 212, 154, 0.35); }
  50%      { box-shadow: 0 0 32px 8px rgba(245, 212, 154, 0.55); }
}
...
bloom は scale を伴うが、**位置移動なし・one-time** という条件下で worldview「fade extended」として許容される(「物が動く」モーションではなく「現れる」モーションの一種)。glow-pulse は breath の「光バージョン」、fade-in-soft は bloom の「小スケール pop-in 版」と捉えると体系的に整理しやすい。
