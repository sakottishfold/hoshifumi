...
| Token | cubic-bezier | Tailwind class | 性質 |
|---|---|---|---|
...
| Use case | Property | Duration | Easing | 備考 |
|---|---|---|---|---|
...
### Transition utility(主に hover / focus / active)
...
| NG | 代わりに |
|---|---|
| Duration < 100ms(ほぼ無視できるが CPU 食う) | 100ms 以上、または animation 自体を削除 |
| Duration > 1500ms(breath loop 以外で) | 1000ms 以下に収める、または出現自体を rethink |
...
| **`docs/DESIGN.md` §7 Do's and Don'ts** | コンポーネントレベルでの motion 適用例(ボタン active state 等) |
