...
## ADR-025: テンプレートをユーザー単位の設定にする(inline switcher 廃止、onboarding 選択 + 設定変更)

- **Date**: 2026-05-23
- **Status**: Accepted
- **Supersedes**: ─
- **Resolves**: ─
- **Related spec**: `docs/specs/2026-05-23-template-as-user-setting-design.md`
- **Affects**: `docs/specs/2026-05-21-additional-templates-design.md` §7.1(inline switcher モデルを覆す)

### 背景

spec `2026-05-21-additional-templates-design.md` §7.1 で「`/today` step 0 に `TemplateSwitcher`(灰色ピル)を出し、毎日 inline で選び替え可能、前回値が sticky に引き継がれる」モデルを採用した。2026-05-23 のオーナーセルフテストで「テンプレート機能がある部分がわかりにくすぎる」── 具体的にはこのピル(`ほしふみ ▾`)の存在に気づかず、押せるとも認識されなかった、という観察が共有された。原因は (1) ピルが小さく灰色で「操作可能」に見えないこと、(2) basic テンプレの表示名「ほしふみ」がアプリ名と同じで、ピルがブランド表示に見えること、(3) テンプレを選んだ結果(Q2 文言が変わる)が Q2 に到達するまで見えないこと。

テンプレは「日記のジャンル」── 一度決めて基本固定する性質のもので、毎晩選び替えるものではない。

### 決定

テンプレートを **ユーザー単位の設定**(`profiles.template_name`、nullable)にする。

- 初回ログイン後、`profiles.template_name` が NULL(onboarding 未完了)なら `/onboarding` へ自動誘導。1画面・5択タップ・即完了で1つ選ぶ → `setTemplate` server action で保存 → `/today` へ
- 変更口は `/settings` の「日記のテンプレート」カードのみ(`TemplatePicker` を inline 展開)
- `/today` から `TemplateSwitcher` および `getLastUsedTemplate`(sticky last-used)を撤去
- `entries.template_name` は引き続きエントリごとに保存(`submitEntry` が `profiles.template_name` の現在値を焼き込む)── 過去エントリは後でテンプレを変えても当時の Q2 文言で表示できる
- basic テンプレの `displayName` を「ほしふみ」→「**きほん**」、`description` を「体・できごと・明日へ」→「ジャンルを決めずに、ふつうに置く」に改名(他テンプレと粒度を揃え、アプリ名との衝突を解消)

### 根拠

- inline ピルが気づかれない問題は配置や色の微調整では解決しない ── テンプレを毎晩選ぶ前提が間違いで、UI を消すのが正解
- ジャーナル画面は問いに集中させる(worldview「夜、ふとんから星を見上げる」の受動的姿勢に沿う)
- 初回 onboarding 1画面・1タップは儀式の穏やかさを壊さない範囲の摩擦
- 過去エントリは `entries.template_name` に焼き込まれているため、設定変更は過去の表示に影響しない(`EntryDetail` が entry 自身のテンプレを読む既存挙動)
- 追加テンプレート機能(5テンプレ体制)自体は廃止しない ── 変えるのは「いつ・どこで選ぶか」のみ

### 影響

- DB: `profiles.template_name`(text、nullable、default なし)を追加(migration `20260523010000_profile_template.sql`)。既存 profile は「直近エントリの template_name、無ければ basic」で backfill
- 新規ルート: `app/onboarding/page.tsx` + `OnboardingPicker`(client wrapper)
- 新規コンポーネント: `components/TemplatePicker.tsx`(onboarding と設定で共有)
- 新規 server action: `lib/server-actions/profile.ts` の `setTemplate(templateName)`
- `app/today/page.tsx`: `profiles.template_name` を読み、NULL なら `/onboarding` へ redirect
- `app/today/_components/QuestionFlow.tsx`: `TemplateSwitcher` import / render と `templateName` state を撤去
- `app/today/_components/TemplateSwitcher.tsx`: **削除**
- `lib/server-actions/entries.ts`: `getLastUsedTemplate()` を削除
- `app/settings/page.tsx` + `_components/TemplateSetting.tsx`: 「日記のテンプレート」カード追加
- basic テンプレの表示名・説明を改名
- 先行 spec `2026-05-21-additional-templates-design.md` §7.1 を本 ADR が覆す(supersede)。spec 本体は履歴として残す
- ADR-008(無罰)/ ADR-019(worldview)と矛盾しない

### 未解決の論点

- onboarding コピー「どんな夜を綴る?」は仮確定。Phase 1 セルフテストで違和感があれば調整
- カスタムテンプレート(ユーザーが Q2 文言を自作)は v1.1+ の検討事項(本 ADR では扱わない)
